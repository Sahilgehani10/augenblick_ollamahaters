...
export const TextEditor = () => {
  ...
  const wrapperRef = useCallback((wrapper: HTMLDivElement) => {
    ...
    // Track cursor position for autocomplete positioning
...
useEffect(() => {
  ...
  // Autocomplete suggestions via Groq API
  ...
  return (
    <div style={styles.editorContainer}>
        ...
        {activeUsers.length > 0 ? (
          activeUsers.map((user) => (
            <div key={user.userId} style={styles.userBadge}>
              <img src={user.imageUrl} alt={user.name} style={styles.userAvatar} />
      ...
      {autocompleteOpen && autocompleteSuggestions.length > 0 && (
        <div style={{ ...styles.autocompleteContainer, top: position.top, left: position.left }}>
